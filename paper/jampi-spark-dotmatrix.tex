...
\affiliation{\textsuperscript{1}\textit{Hewlett Packard Enterprise Co.,  Miami, FL.}} % Author affiliation
...
Spark's Barrier Mode is part of Project Hydrogen\cite{projecthydrogensite} that introduces gang scheduling on top of MapReduce execution model to support distributed deep learning executed or embedded as Spark steps. The current implementation ensures that all tasks (limited to \texttt{mapPartitions}) executed in the same time, and collectively cancels and restarts them in case of failures. In addition to the true parallell execution, the worker hostnames, partition IDs are accessible inside the tasks as well as a \texttt{barrier} call, similar to MPI's \texttt{MPI\_Barrier} function \cite{projecthydrogenpres}.

While these functionalities are sufficient to support Barrier's mode primary use case: namely executing embedded MPI or other, foreign (non-JVM and non-Spark) steps inside a Spark application, it does not provide any inter-task communication primitive to implement the same algoritms inside JVM/Spark native steps. In fact, Barrier Execution Mode's design documentation clearly sets this as a non-goal, stating that beyond a simple \texttt{BarrierTaskContext.barrier()} call no intra-communication functionality will be part of the implementation, as it is assumed to be handled by the user program. We see this as a clear showstopper for Spark distributed deep learning adoption, forcing the executions outside of Spark boundaries.
...
The MPI version of the algorithm relies on MPI's cartesian topology. After setting up the 2D communication grid with \texttt{MPI\_Cart\_create}, processors exchange data with their neighbors by calling \texttt{MPI\_Sendrecv\_replace}. In the main loop, each processor executes a local dot product calculation, then shift the results horizontally for matrix  $\mathbf{A}$  and vertically for matrix  $\mathbf{B}$. In our benchmarks we used \texttt{MPICH} version 3.3.2 as MPI implementation.
...
JAMPI is a from scratch built, native Scala implementation of the Cannon's algorithm. For message passing, we built an \texttt{nio} based asynchronous message passing library, that mimics MPI's cartesian topology and send-recv-replace functionality. To avoid unnecessary memory copies and to optimize performance for both throughput and latency, our \texttt{PeerMessage} object allocates fixed 8MB off-heap buffers for both sending and receiving data. Sending and receiving network operations are executed in parallel asynchronously.

The matrix multiplication is embedded into a Barrier Execution Task, and it takes the number of partitions,  the local partition ID, the hostnames for the other partitions (\texttt{address} from \texttt{BarrierTaskContext.getTaskInfos()}) along with the local matrix pairs from the \texttt{RDD}.  
...
In order to achieve performance in par with the optimized MPI implementation for local dot product steps, we used JVM's native vector intricts and superword optimization capabilities for both Jampi and MLlib Spark applications benchmarks. 
The latest, most comprehensive vectorization support in JVM is found in \texttt{Vector API} module, part of OpenJDK's \emph{Project Panama}. While the \texttt{Vector API} is currently in incubator, we considered stable enough to use it for both the Spark platform and application code. 

For fair benchmarking, we avoided using \texttt{Vector<>} objects or advanced methods like manual unrolling. While these techniques could potentially further improve performance, our goals were to compare the distributed algoritms' performance with the same CPU opcodes used in local matrix multiplications. From the JIT compiler outputs, we confirmed that both Spark applications were using \texttt{vfmadd231sd} as in the GCC compiled MPI version.

To use the new vector intricts features, we built a custom OpenJDK package from panama/dev branch tip (\texttt{dev-442a69af7bad}).
...
The applied JVM flags were \texttt{--add-modules jdk.incubator.vector} and \texttt{ -XX:TypeProfileLevel=121} for both JAMPI and MLlib applilcations. 

% subsection vector_panama (end)

\subsection{Apache Spark MLlib}

We used Apache Spark MLlib's built-in \texttt{BlockMatrix.multiply} as a baseline to compare with JAMPIs speed and resource usage. As MLlib's implementation is prone to be faster with more partitions than the total number of worker cores, we executed two tests for each scenario: one with number of partitions equal to worker cores and one where the number of partitions were equal to four times of the total worker cores. For the latter scenario we refer as \emph{overpartitiong}.

\subsection{Test protocols} % (fold)
\label{sub:test_protocols}

All tests are executed Amazon Web Services EC2 instances using \texttt{m5} instance types with Intel(R) Xeon(R) Platinum 8175M CPUs and 4GB / cores RAM. Apache Spark version was 3.0.0-preview2 with separate master node. Driver process was initated from the master node and its resource consumption is not included in the results. For single code tests we disabled the second CPU core in the VM.  


\begin{center}
	\begin{tabular}{ |c|c|c|c| } 
	 \hline
	 total worker cores & instance type & no. nodes & partitions  \\ 
	 \hline\hline
	 1 & m5.large & 1 & 1 \\ 
	 \hline
	 16 & m5.xlarge & 4 & 4 \\ 
...
Applications reported only the dot product execution time with a single one value reducer (\texttt{avg}) to trigger \texttt{RDD} reduce/collection on Spark without moving substantial amount of data to the driver process. This excluded the MPI and Spark application startup times but included the time required to establish a Barrer Task Step during \texttt{RDD} reduce. We used 64bit floating point data types for matrix elements. 

All test cases where executed at least ten times while execution times, CPU and memory consumptions were captured. 
