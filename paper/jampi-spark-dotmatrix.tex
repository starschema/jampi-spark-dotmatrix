...
More generally, it holds that the running time $T(n, p)$ for problem size $n$ and $p$ processors scales by a constant factor $c$, so that
...
From Equation~\eqref{eq:general_runtime}, it follows that
...
It follows thus from Equation~\eqref{eq:scale_factor} and $M(n) = n^2$ that
